...
{- | Records in the `ConstraintsState` (and in particular, in `cs_asserts`
 ...
 The way these instructions deal with the AP register is deferred to mkApConstraints.
...
{- | Records in the `ConstraintsState` (and in particular, in `cs_asserts`
field) the assertions corresponding with the semantics of `assert_eq` and
`call`, specifically pertaining to the way they influence the AP register. -}
...
{- | Records in the `ConstraintsState` (and in particular, in `cs_asserts`
field) the assertions corresponding with the semantics of builtins.

Importantly, we do not do any processing of builtins in modules that optimise
preconditions of functions, even in spite of the fact that builtins do have preconditions.

This entails that a failure in a precondition of a builtin for a function will be reflected
as a failure in the 'main' module for said function, not in the module that optimises its pre. -}
...
{- | Contrats for builtins ensure that their corresponding builtin pointers stay within reasonable ranges. -}
getBuiltinContract ::
...
{- | Abstracted calls handle builtins by their builtin contract - cf. getBuiltinContract.

For inlined calls, one needs to make sure to finish inlining (i.e. hitting the corresponding
'ret') before checking whether the builtin contract is fulfilled.

For assert_eq, one needs to track updates to builtin pointers to ensure they do not get out of range. -}
...
{- | In as cryptic as this function might be, it follows from the semantics of the Cairo language.
Do not worry about it and do not touch it. -}
